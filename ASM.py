...
def smooth_raw_data_RDS(smooth_field, speed_raw, dx, dt, smooth_x_window=1, smooth_t_window=180):
    EGTF_speed = pd.DataFrame(smooth_field)
    EGTF_speed.columns = ['t_index', 'x_index', 'speed']
    EGTF_speed['t'] = 0.02 * EGTF_speed['t_index']
    EGTF_speed['x'] = 4 * EGTF_speed['x_index'] + 58.7

    rds_speed = pd.DataFrame(speed_raw)
    rds_speed.columns = ['t_index', 'x_index', 'speed']
    rds_speed['t'] = dt * rds_speed['t_index']
    rds_speed['x'] = dx * rds_speed['x_index'] + 58.7
    speed_raw_df = rds_speed.dropna().copy()
    EGTF_speed['EGTF'] = EGTF_speed.apply(lambda v: EGTF(v.x, v.t, smooth_x_window, smooth_t_window, speed_raw_df),
                                          axis=1)
    EGTF_speed.columns = ['t', 'x', 'raw_speed', 'time', 'milemarker', 'speed']
    return EGTF_speed